...
2. Appeler `aret_restore` pour récupérer doctrine, version du Store et contexte chaud.
3. Appeler `aret_get_front` avant de modifier une orientation active ou une brique référencée par le Front.
4. Considérer la mémoire SQLite comme la source canonique ; le texte de conversation n’est jamais une preuve de l’état du projet.
...
- Après une mutation significative, relire la ressource ou la roadmap concernée afin de vérifier l’état canonique réellement enregistré.
