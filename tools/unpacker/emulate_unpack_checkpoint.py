import builtins, threading, sys, os, time, struct, collections, json, pickle
...
def save_ckpt(uc, ah):
    ...
    for (begin,end,perm) in uc.mem_regions():
        size=end-begin+1
        try: data=bytes(uc.mem_read(begin,size))
        except Exception: continue
        open(f"{CKPT_DIR}/{begin:08x}_{size:08x}.bin","wb").write(data)
        regions.append((begin,size,perm))
    RN=['EAX','EBX','ECX','EDX','ESI','EDI','EBP','ESP','EIP','EFLAGS']
    regs={r:uc.reg_read(getattr(__import__('unicorn').x86_const,'UC_X86_REG_'+r)) for r in RN}
    ah_keys=['module_handle_offset','dynamic_mem_offset','allocated_chunks','alloc_sizes','_fakes','module_handles','load_library_counter','heaps','next_heap_handle','next_hook_offset','module_for_function']
    ah_state={k:getattr(ah,k) for k in ah_keys if hasattr(ah,k)}
    pickle.dump({'regions':regions,'regs':regs,'ah':ah_state,'cnt':cnt[0]}, open(f"{CKPT_DIR}/meta.pkl","wb"))
    sys.stdout.write(f"[CKPT] saved at cnt={cnt[0]} regions={len(regions)}\n"); sys.stdout.flush()
...
def lean_code(self, uc, address, size, user_data):
    ...
    if cnt[0]==CKPT_AT:
        try: save_ckpt(uc, self.apicall_handler)
        except Exception as e: sys.stdout.write(f"[CKPT] err {e}\n"); sys.stdout.flush()
...
def patched_invalid(self,uc,access,address,size,value,user_data):
    ...
    if faults[0]<=40:
        ...
        sys.stdout.write(f"MEMINVALID#{faults[0]} acc={access} addr={hex(address)} sz={size} eip={hex(eip)} cnt={cnt[0]}\n")
        for a in list(ring)[-6:]:
