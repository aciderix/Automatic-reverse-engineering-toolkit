...
NTSTATUS WINAPI RtlpNtCreateKey(PHANDLE retkey, ACCESS_MASK access, const OBJECT_ATTRIBUTES *attr, ULONG TitleIndex, const UNICODE_STRING *class, ULONG options, PULONG dispos);
...
NTSTATUS WINAPI RtlpNtQueryValueKey(HANDLE handle, ULONG *result_type, PBYTE dest, DWORD *result_len, void *unknown);
NTSTATUS WINAPI RtlpNtSetValueKey(HANDLE hkey, ULONG type, const void *data, ULONG count);
NTSTATUS WINAPI RtlFormatCurrentUserKeyPath(IN OUT PUNICODE_STRING KeyPath);
NTSTATUS WINAPI RtlOpenCurrentUser(IN ACCESS_MASK DesiredAccess, /* [in] */ OUT PHANDLE KeyHandle);
NTSTATUS WINAPI RtlQueryRegistryValues(IN ULONG RelativeTo, IN PCWSTR Path, IN PRTL_QUERY_REGISTRY_TABLE QueryTable, IN PVOID Context, IN PVOID Environment OPTIONAL);
...
NTSTATUS WINAPI RtlWriteRegistryValue(ULONG RelativeTo, PCWSTR path, PCWSTR name, ULONG type, PVOID data, ULONG length);
