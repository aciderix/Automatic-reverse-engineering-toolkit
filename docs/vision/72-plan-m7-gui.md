...
| **G5** ✅ (partiel) | **Dialogs + MessageBox** : **MessageBoxA/W = -1 display-free** (repli sound = Wine-sans-écran, fait). *Reste : DialogBoxParamA/CreateDialogParamA parsent le **DLGTEMPLATE** → contrôles + pompe modale ; EndDialog ; GetDlgItem/Set-GetDlgItemTextA.* | MessageBoxA 37 ✅ ; DialogBoxParamA 17, CreateDialogParamA 23, EndDialog 20, GetDlgItem 33 | `user32_messagebox` (`.nodisplay` → -1 vs Wine-sans-écran) ✅ ; puis fixture dialogue (template .rc + WM_INITDIALOG + EndDialog) vs Wine sous Xvfb |
