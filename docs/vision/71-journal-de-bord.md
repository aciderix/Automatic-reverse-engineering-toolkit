...
- **À FAIRE dès reconnexion MMU** : `aret_run_oracle(cpudiff, promote)` → preuve canonique admissible attachée à KN-0049 ; enregistrer KN-0049 en MMU. **Suite** : min/max entier packé (`pminub`/`pmaxub`/`pminsw`/`pmaxsw`), puis `ud2`→trap, mul/div 8/16-bit, `fldenv`/`fnstenv` (x87 env, §0-sensible arrondi → prudent).
