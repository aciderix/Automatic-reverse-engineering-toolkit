...
- **À FAIRE dès reconnexion MMU** : `aret_run_oracle(cpudiff, promote)` → preuves canoniques admissibles (cmppd + min/max) attachées à KN-0049 ; enregistrer KN-0049 en MMU. **Suite** : `ud2`→trap, `mul`/`div` 8/16-bit, `fldenv`/`fnstenv` (x87 env, §0-sensible arrondi → prudent), puis lift end-to-end d'un outil h5 vs Wine.
