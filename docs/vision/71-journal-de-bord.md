...
### 2026-09-01 — [LIFT][INT][§0 cause générale] **`mul`/`div`/`imul`/`idiv` 8-bit et 16-bit (formes 1-opérande) liftés** — KN-0052
Suite du mur `libhdf5` (option 1). Le lifter ne gérait que les formes 32/64-bit des `mul/div/imul/idiv` 1-opérande ; 8/16-bit tombait en `asm()` → abort (mur : `mul byte`, `div byte`, `div word`). **Cause générale**, pas une rustine HDF5 : ces formes apparaissent partout (ex. sqlite `h % 23`).
- **Sémantique x86 exacte** : **8-bit** n'utilise QUE `ax` (`al`×r/m8→`ax` ; `ax`/r/m8→`al`=quotient, `ah`=reste, donc `ax`=(reste<<8)|quotient) ; **16-bit** utilise `dx:ax` comme la forme 32-bit en plus étroit. `div`/`idiv` **trappent #DE** (diviseur nul OU quotient qui déborde la destination) — jamais une troncature silencieuse (§0).
- **Fix additif** (hash `19acad982194bf07` **inchangé**) : `src/ir/lift.rs` arms `w==16` et `w==8` (avant le garde `w!=32`) ; helpers C `__ix_udiv8/umod8/idiv8/imod8` + `__ix_udiv16/umod16/idiv16/imod16` (`src/emit/mod.rs`, même style trap que `__ix_udiv32`) ; références + 8 encodages corpus (mul/imul/div/idiv en cl et cx) dans `src/cpudiff.rs`.
- **Portes LOCALES vertes** : **cpudiff PASS (6/0)** (les 8/16-bit vs Unicorn, y compris les cas #DE que le harnais skippe comme Unicorn) ; **difftest 272/272**, **funcdiff 0 divergence** (et davantage de fonctions scorées : du code qui abortait lifte désormais), **transpile-hash 4/4** inchangé. Effet : wall `libhdf5-310.dll` **30 → 20 sites**. Preuve canonique cpudiff MCP à enregistrer.
- **Reste au mur `libhdf5` (20 sites)** : `ud2` (8, trap #UD déjà sûr) et `fldenv`/`fnstenv` (12, environnement x87 = arrondi §0-sensible, à traiter avec soin).

### 2026-09-01 — [HLE][HDF5][§0 bornage] **`strtok_s` shimmé ; HDF5 fonctionnel sous ARET borné par C99 `fenv` (`fnstenv`/`fldenv` = flags d'exception FP)** — KN-0053
Option A (faire tourner HDF5 sous ARET, preuve fonctionnelle), §2 reproduire en exécutant la fixture round-trip liftée.
- **Livré** : le 1er vrai mur côté ARET n'est PAS `SHGetFolderPathW` (c'était un artefact de résolution d'env côté *Wine*) mais l'import msvcrt **`strtok_s`**. Shimmé : `aret_strtok_s` (`runtime/aret_hle/aret_crt.c`) via POSIX `strtok_r` + round-trip 32-bit du contexte guest (adresse guest == pointeur host, map 1:1 low-4G ; macros `AP`/`RP`). Le mécanisme weak-stub/strong-override route l'import vers le shim (aucune table). **Preuve : winediff `crt_strtok_s` PASS** (2 tokenisations INTERLEAVÉES + cas limites) = ARET == msvcrt Wine byte-exact. Additif HLE : ni hash transpile ni difftest affectés.
- **Borné §0** : après `strtok_s`, le mur devient `fnstenv`/`fldenv` **atteint au runtime**. Désassemblage (libhdf5 `0x71116085..0x71116146`) = l'implémentation **C99 `<fenv.h>`** : `fnstenv;orw $flag,status;fldenv`=`feraiseexcept`, `fnstenv;and mask;fldenv;stmxcsr;…;ldmxcsr`=`feclearexcept`, `fnstenv (eax);fnclex`=`fegetexceptflag`. Ces fonctions manipulent les **flags d'exception FP** (status word), pas l'arrondi. ARET ne suit PAS l'état d'exception FP ; un bloc canonique « aucune exception » serait un **faux silencieux** dès qu'une conversion lève overflow/invalid (que HDF5 teste dans H5T) → §0 = reste **abort**. **HDF5 fonctionnel sous ARET = bloqué par C99 `fenv`** (suivi des exceptions FP = chantier x87/SSE majeur §0-sensible). Le winediff byte-exact end-to-end restait de toute façon bloqué côté Wine (`SHGetFolderPathW` stub, KN-0050).
- **Bilan front LIFT-HDF5** : instructions générales tractables toutes prouvées (P-0031, P-0032) ; `strtok_s` livré (winediff) ; HDF5 fonctionnel/end-to-end borné (fenv + oracle Wine). Prochain incrément HDF5 réel = via l'oracle Windows GH Actions, ou un sous-chantier « suivi exceptions FP ».

---

## GEL DE REPRISE — 2026-09-05 (MCP intermittent ; source canonique = ARET-MMU, KN-0054→KN-0086 + fixes ci-dessous)

> Ce bloc fige dans le journal narratif tout ce qui a été fait **depuis la dernière entrée (2026-09-01, KN-0053)**, par sécurité pendant une phase d'instabilité du serveur MCP. **La MMU (SQLite, KN/preuves) reste la source canonique** ; ce gel est un doublon lisible, pas un substitut. Les entrées marquées « MMU canonique » sont prouvées par oracle admissible côté MMU ; celles marquées « vérifié labo » sont validées par exécution réelle (byte-compare Wine, régression) mais leur preuve MCP restait à (ré)enregistrer au moment du gel.

### 2026-09-02→03 — [PONT] KN-0054→KN-0063 (TLS statique .tls, SIMD SSE2/MMX pixman, différés getch/ftime, maturité GTK) — détail canonique en MMU
Travaux enregistrés en MMU entre KN-0053 et l'arc gdk-pixbuf, non re-détaillés ici pour ne rien inventer (MCP indisponible au gel) : TLS statique Windows (`.tls`, cause générale d'un crash cairo/pixman) + industrialisation fixture TLS winediff permanente ; mur SIMD SSE2/MMX (fast-paths pixman) ; différés `_getch`/`_ftime` ; test de maturité GTK (chargement dynamique de DLL liftées). **Voir KN-0054..KN-0063 en MMU pour le détail technique et les preuves.**

### 2026-09-03→04 — [RECOV][LIFT][EH] Arc gdk-pixbuf : 3 causes générales + stub GDI+ → PNG décodé e2e byte-identique Wine — KN-0064/0069/0074/0077/0079 (MMU canonique)
Test de maturité « jusqu'où ARET va sur une vraie lib » mené sur la fermeture de décodage PNG de gdk-pixbuf (glib/gobject/gio/gmodule/gdk_pixbuf/png16/zlib + codecs). Quatre murs franchis, tous **causes générales prouvées** :
- **KN-0064 — over-read de jump-table (garde `cmp` sur opérande mémoire)** (commit 184ff7c) : une table de saut était sur-lue faute de reconnaître une garde `cmp` dont l'opérande est en mémoire → explosion du codegen gdk-pixbuf. Preuves P-0062 (transpilediff hash) + P-0063 (winediff corpus).
- **KN-0074 — faux départs de fonction** (garde false-start, `src/analysis/mod.rs`, commit f800eff) : des trampolines/continuations (`ret` en fallthrough d'un `je`, désync de décodage x86) étaient pris pour des entrées de fonction → 2ᵉ cause d'explosion (libLerc). Critères : entrée qui ouvre un Jcc OU précédée d'un CondJump, hors call-targets/cxx-continuations. Preuves P-0068/P-0069.
- **KN-0077 — ordre d'initialisation des DLL par dépendances** (`src/loader/mod.rs`, commit 7d43e93) : `dependency_init_order` topologique stable (casse les cycles déterministiquement) ; `dll_inits`/`tls_inits`/`ctor_list` itèrent dans l'ordre de dépendances comme le loader Windows. Champ `imports` ajouté à `LoadedModule`. Preuves P-0070/P-0071 + 2 tests unitaires.
- **KN-0069 — reprise EH : threading des callee-saved vers le landing pad/continuation** : le dispatcher EH ne threadait qu'esp/ebp/obj/sel ; une continuation lisant un local resté en registre callee-saved (ex. compteur de boucle en ebx) reprenait à 0. Fix : enregistrer ebx/esi/edi live à chaque call gardé et les rendre à la reprise.
- **Stub sound `GdiplusStartup`/`GdiplusShutdown`** (commit 0d6ed6e) : mesuré que seule l'init GDI+ est appelée sur le chemin PNG → stub 1-fonction suffisant (SuppressBackgroundThread → `aret_partial`).
- **Résultat KN-0079 (MMU canonique, PROVEN par 3 preuves)** : gdk-pixbuf décode le PNG e2e sous ARET = **byte-identique à l'oracle Wine** (`w=4 h=3 nc=3 hash=2efc69df`). P-0072 (transpilediff, hash 19acad982194bf07 inchangé), P-0073 (funcdiff 0 divergence), P-0074 (winediff fixture permanente `glib_gdkpng`). Fixture + `bench/glib_fetch.sh` étendu (commit b64a79e). **Test de maturité gdk-pixbuf ABOUTI et industrialisé.**

### 2026-09-04 — [EH][§0] exception_ptr C++11 : faux silencieux corrigé (en-tête __cxa_exception + refcount) — KN-0080 (MMU canonique)
**Mur §0 mesuré** : `std::current_exception()` renvoyait un exception_ptr NUL silencieusement (`captured=0` vs Wine `1`), puis `std::rethrow_exception(null)` crashait — sortie incorrecte présentée comme correcte. **Cause générale** (classe entière : current_exception, rethrow_exception, exception_ptr copie/dtor, nested, promise::set_exception) : le compteur de références qui garde l'objet vivant vit dans un **en-tête `__cxa_exception` AVANT l'objet** (mingw libstdc++ mesuré : `__cxa_allocate_exception` = `malloc(size+0x60)`, objet à base+0x60, referenceCount à objet-0x60, exceptionDestructor à objet-0x4c) que le modèle fermé d'ARET n'allouait pas → le lifted `_M_addref`/`_M_release` (`lock addl/subl -0x60(obj)`) écrivait sous l'allocation. **Fix** (`runtime/aret_hle/aret_hle.c` + `src/loader/mod.rs`, commit 4fa90ba) : réserver l'en-tête 0x60 ; `__cxa_throw` sème refcount=1 + dtor + tinfo ; `__cxa_end_catch` sensible au compteur ; host-back `std::current_exception`/`std::rethrow_exception` (noms manglés dans `is_eh_runtime_symbol`) car leurs corps liftés exigent `__cxa_get_globals`/une dependent exception. **Preuves** : P-0075 (transpilediff hash inchangé) + P-0076 (winediff `lift_exception_ptr` byte-identique Wine). Régression 6 fixtures EH + 83 tests.

### 2026-09-04 — [EH][RTTI] nested_exception + dynamic_cast fonctionnent (extension exception_ptr) — KN-0081 (MMU canonique)
`std::throw_with_nested`/`std::rethrow_if_nested` (+ `dynamic_cast<nested_exception*>` RTTI) marchent byte-identiques à Wine SANS nouveau mur, grâce au fix KN-0080 + le `__dynamic_cast` lifté. Verrouillé : fixture `lift_nested_exception` (commit 089ed76). Preuve P-0077 (winediff).

### 2026-09-04 — [THREAD][§0] GetThreadId + schéma thread-id fibers unifié → std::async cross-thread — KN-0082 (MMU canonique)
`std::async`/`std::future` abortaient sur l'import `GetThreadId` non modélisé. **Cause générale** : deux conventions d'ID de thread incohérentes (`GetCurrentThreadId`=getpid pour tout fiber vs `CreateThread` lpThreadId=0x1000+idx). **Fix** (`aret_win32.c`, commit cdbf6c2) : helper `u32_thread_id` (main=getpid, worker=0x1000+idx) partagé par GetCurrentThreadId/GetThreadId/lpThreadId. La propagation d'exception entre fibers coopératifs (worker throw → future stocke via current_exception → get() re-lève) est alors byte-identique Wine. Fixture `lift_async_exception`. Preuves P-0078 (transpilediff) + P-0079 (winediff).

### 2026-09-04 — [EH][§0] catch par base VIRTUELLE : faux catch silencieux corrigé (offset vbase via vtable) — KN-0083 (MMU canonique)
**Mur §0** : `catch(const V&)` d'un objet dont V est base **virtuelle** attrapait le MAUVAIS handler (`catch(...)` en aval) — « caught other » vs Wine « caught V.x=99 ». **Cause** : le matcher de sous-type Itanium sautait les bases virtuelles (offset dans la vtable, pas fixe) ; le commentaire supposait « saut → non-géré → abort », faux dès qu'un handler plus large suit → faux catch silencieux. **Fix** (`aret_hle.c`, commit b29d1a2) : threader le pointeur objet dans `aret_gnu_type_match_adj` et lire l'offset vbase depuis la vtable (Itanium : `off = *(ptrdiff_t*)(*(void**)subobj + (offset_flags>>8))`). Fixture `lift_vbase_catch`. Preuves P-0080 (transpilediff hash inchangé) + P-0081 (winediff).

### 2026-09-04 — [EH][RTTI] catch-by-value + dynamic_cast complexe : déjà fonctionnels, verrouillés — KN-0084 (MMU canonique)
Deux derniers bords EH/RTTI sondés (§0.5 mesure) : `catch(Msg)` par valeur (copie non-triviale) et `__dynamic_cast` complexe (down/cross/vbase up-cast sur losange polymorphe) marchent déjà byte-identiques Wine, aucun bug. Verrouillés : fixtures `lift_catch_byval`, `lift_dynamic_cast` (commit abd1d49). Preuves P-0082 + P-0083 (winediff). **Vein C++ EH/RTTI lourd couvert : 6 fixtures permanentes, 2 faux silencieux §0 réels corrigés (0080, 0083).**

### 2026-09-04→05 — [LIFT][ISA][§0] Clamp CPUID cohérent : n'annoncer que les ISA entièrement liftées — KN-0085 (MMU canonique)
**Mesure de cible** (`--mode walls` sur zstd/xz/openssl/libdeflate) : libdeflate-gzip = 599 instructions vectorielles distinctes, mais majoritairement dead-code (CPUID clampe déjà AVX/AVX-512). **Bug général** : le clamp `__ix_cpuid` (`src/emit/mod.rs`) **annonçait** SSE3/SSSE3/PCLMULQDQ/AES-NI/BMI2/SHA sans les modéliser → un binaire dispatchant dessus (CRC PCLMULQDQ, crypto GCM, bitreader BMI2) **abortait** au lieu de retomber sur un chemin scalaire exécutable. **Fix** (commit a330426) : étendre le masque pour n'annoncer que les familles entièrement liftées (leaf1 ecx += SSE3/PCLMULQDQ/SSSE3/MOVBE/AES ; leaf7 ebx += BMI1/BMI2/ADX/SHA) + ajout `__p___winitenv`. **Strictement §0-sain** : au pire même abort, jamais un faux. Résultat : **libdeflate-gzip compresse byte-identique à Wine** (avant : abort PCLMULQDQ). Fixture permanente `cpuid_feature_fallback` (BMI2 derrière `__builtin_cpu_supports`, fallback scalaire). Preuves P-0084 (winediff) + P-0085 (transpilediff hash inchangé).

### 2026-09-05 — [TOOLING] Réduction taille sortie transpile (sections.bin auto + ARET_CLEAN_INTERMEDIATES + tmpfs) — KN-0086 (MMU canonique)
Une out-dir d'auto-lift libstdc++ (~24 Mo d'entrée) faisait ~173 Mo (.c 53 + .o 50 + exe 49 + sections.bin 23 dupliqué par aret_layout.S.o). **Fix post-link** (`src/builder/mod.rs`, commit 0c0bf37, n'affecte pas le code transpilé) : suppression systématique de `sections.bin` (173→151 Mo, risque nul) ; `ARET_CLEAN_INTERMEDIATES=1` efface aussi .c/.o (173→50 Mo, objcache garde ses copies hashées) ; mode mémoire via `--out-dir /dev/shm` (build RAM, testé). Hash 19acad982194bf07 inchangé (4/4). Preuve P-0085 (partagée).

### 2026-09-05 — [HLE-FILE][§0] fread doit poser le flag EOF sur une lecture courte — (vérifié labo, KN + preuve MCP à enregistrer)
**Mur §0 mesuré** via zstd : `zstd -19` sous ARET abortait `Unexpected short read` (error 37) là où Wine réussit. **Cause générale** : `aret_fread` boucle pour remplir la demande mais, quand `read()` atteint EOF en milieu de buffer, il `break` **sans poser `_IOEOF`** → l'idiome standard `fread`+`feof` (lire un bloc > reste, puis `feof` pour distinguer EOF d'une erreur) voit lecture courte + `feof()==0` = erreur dure. **Fix** (`runtime/aret_hle/aret_hle.c`, commit 2ada4bf) : sur `read()<=0`, poser `_IOEOF` (0x10) à EOF / `_IOERR` (0x20) sur erreur, comme le CRT/`pull_byte`. Résultat : **zstd -19 compresse maintenant** (sortie valide, décode == original via zstd natif). Fixture permanente `crt_fread_eof` (byte-identique Wine 1/1 ; divergeait avant). Régression : hash inchangé (4/4), 6 fixtures fichier/CRT 1/1, 83 tests. **Preuve canonique MCP (winediff + transpilediff) à enregistrer au retour du MMU.**

### 2026-09-04→05 — [INFRA][MCP] Transport MCP en HTTP local persistant (anti-coupure) — (vérifié labo, opérationnel)
**Mesure** : les coupures MCP (CONNECT_TIMEOUT) ne venaient PAS du venv (déjà persistant via uv ; serveur prêt en ~1 s, handshake MCP réel OK) mais de la **charge du conteneur** pendant le spawn+handshake stdio par connexion. **Fix** : `scripts/launch_aret_mcp_http.sh` (lanceur HTTP persistant, idempotent, détaché via `setsid`), lancé par le hook SessionStart ; `.mcp.json` bascule en `{ type: http, url: http://127.0.0.1:8765/mcp }` (stdio conservé en repli dans `integration/mcp.stdio.snippet.json`). Commits bbdbc2b + 2ada4bf. **Validé de bout en bout** (log : le harness s'est connecté en HTTP, `POST /mcp → 200 OK`) ; le fix `setsid` corrige la longévité du serveur dans le conteneur éphémère. Prend effet au démarrage de session.
