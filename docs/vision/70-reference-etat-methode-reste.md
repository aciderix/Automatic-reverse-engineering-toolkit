...
bash bench/winediff.sh              # axe 2 vs Wine (164/164 ; gdi_uifont peut être rouge = env fontconfig i386)
...
**164/164** (le seul rouge possible = `gdi_uifont`, **environnemental** : fontconfig i386, orthogonal au code) · cpudiff vert (per-instruction + séquences génératives) · funcdiff corpus **0 divergence** (lift **~20,6k** scorées /
