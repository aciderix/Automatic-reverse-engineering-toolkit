/* STATIC control paints: SS_LEFT label fills COLOR_WINDOW (dialog bg, measured vs Wine)
...
int main(void) {
    ...
    wc.lpfnWndProc = WndProc; wc.hInstance = GetModuleHandleA(NULL); wc.lpszClassName = "EPar";
    RegisterClassA(&wc);
    HWND par = CreateWindowExA(0, "EPar", "p", WS_OVERLAPPEDWINDOW, 0, 0, 200, 200, NULL, NULL, wc.hInstance, NULL);
    const int W = 80, H = 20;
    HWND st = CreateWindowExA(0, "EDIT", "Text", WS_CHILD | WS_VISIBLE | ES_LEFT, 0, 0, W, H, par, (HMENU)1, wc.hInstance, NULL);
...
#define SC(i)    (GetSysColor(i) & 0xFFFFFF)
    ...
    for (int i = 0; i < W * H; i++) { unsigned c = px[i] & 0xFFFFFF;
        if (c == 0xFF00FF) magenta++; else if (c != SC(COLOR_WINDOW)) textpix++; }
    printf("bg=%d magenta=%d text_drawn=%d\n", AT(0, 0) == SC(COLOR_WINDOW), magenta, textpix > 0);
    printf("done\n");
    return 0;
}
