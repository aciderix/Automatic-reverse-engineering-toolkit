...
| `aret_prepare_handoff` | Écriture | Prépare atomiquement le handoff, `last_action`, le checkpoint V1.2 et les curseurs V1.3 ; aucun outil supplémentaire n’est créé. |
...
| Resume Dossier | Cinq domaines de playbook obligatoires **chargés depuis le fichier autoré `config/playbook.md`** (Markdown, ancre `PLAYBOOK_*` par section, jamais ingéré dans SQLite) ; handoff Front atomique, checkpoint déclaratif conditionnel et fenêtre d’observations factuelles dérivés de SQLite. Éditer le playbook ou changer la documentation ne mute jamais la mémoire vivante. |
...
Les hooks Claude Code `SessionStart`, `PreCompact` et `PostCompact` sont installés dans `.claude/settings.json`. `SessionStart` et `PostCompact` **chargent les cinq domaines du playbook depuis le fichier autoré `config/playbook.md`** (source des lois stables, projet-agnostique : on remplace le fichier pour un autre projet ; il n’est jamais ingéré dans SQLite), puis construisent l’état vivant depuis `front_state`, `pipeline_run` et `proof` — assemblant un Resume Dossier à six sections fixes : playbook stable, handoff actif incluant un checkpoint technique déclaratif, adresses pertinentes, capacités/outils/portes avec toolchain et observations factuelles, Git/limites et rituel. Le bootstrap one-off `migration/bootstrap_playbook_v11.py` crée idempotemment les trois fiches dérivées et compactes de méthode, gates et diagnostic, avec relations `DERIVED_FROM` vers leurs sources historiques ; il n’est jamais appelé par un hook ni par `aret_prepare_handoff`. Cette macro écrit atomiquement les quatre champs de handoff, l’action suivante, les adresses chaudes, l’état `NONE` ou `ACTIVE` du checkpoint, les cinq faits techniques, `last_action` et les deux curseurs de frontière des observations. `NONE` est le seul état admissible lorsqu’aucun geste n’est actif ; aucun checkpoint ni curseur ne peut être rétrospectivement fabriqué par migration. V1.3 affiche au plus trois faits persistés après les curseurs : pipeline/proof, verdict, horodatage, adresse, hash d’artefact et paramètres bornés. Ils ne déclarent jamais l’intention de Claude. Une validation qui cite une adresse pipeline/preuve est vérifiée contre le résultat stocké et une contradiction est refusée ; sans adresse, le verdict reste explicitement non vérifié. Aucun document historique n’est injecté massivement ni relu par défaut. Les checkpoints de compaction ne sont écrits que si `ARET_HOOK_WRITE_ENABLED=true`; ils sont des événements d’audit, non des connaissances autoritatives.
