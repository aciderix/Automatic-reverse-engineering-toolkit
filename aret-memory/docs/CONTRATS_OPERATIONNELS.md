...
|---|---|
| Hooks | Lecture seule par défaut. `SessionStart` et `PostCompact` injectent un Resume Dossier V1.3 prêt puis arment une barrière liée à son hash ; les checkpoints d’audit exigent `ARET_HOOK_WRITE_ENABLED=true`. |
...
| Gouvernance V1.4 | Une capacité ARET cataloguée passe par MCP ; le shell sert au laboratoire mais ses sorties ne sont ni faits canoniques ni preuves. Un outil réutilisable ou décisionnel devient une capacité MCP officielle avant usage récurrent. |
...
|---|---|---|
| `SessionStart` | `.claude/hooks/aret-mmu-session-start.sh` | Vérifie et injecte le Resume Dossier V1.3, puis arme une barrière liée à son empreinte. |
| `PreCompact` | `.claude/hooks/aret-mmu-pre-compact.sh` | Retourne Front, audit récent borné et les observations factuelles persistées depuis le handoff ; ne crée aucun checkpoint d’intention. |
| `PostCompact` | `.claude/hooks/aret-mmu-post-compact.sh` | Réinjecte un Resume Dossier V1.3 fraîchement vérifié et réarme une barrière liée à son empreinte ; trace optionnelle de compaction. |
...
`SessionStart` et `PostCompact` n’injectent ni historique massif ni extraits sélectionnés par date. Ils exigent et formatent un **Resume Dossier V1.3** à six sections fixes : playbook stable, handoff actif avec checkpoint technique, adresses pertinentes, capacités/outils/portes incluant la toolchain et les observations MCP factuelles, Git/limites/garde-fous et rituel. Le playbook utilise exclusivement `knowledge` et `knowledge_tag` : ses cinq domaines obligatoires sont `PLAYBOOK_FOUNDATION`, `PLAYBOOK_METHOD`, `PLAYBOOK_ARCHITECTURE` avec `PLAYBOOK_SHARED_STACK`, `PLAYBOOK_GATES` et `PLAYBOOK_TOOLING`, tous sous le tag `CORE_PLAYBOOK`. La migration one-off `migration/bootstrap_playbook_v11.py` ajoute idempotemment trois fiches `RULE` compactes de méthode, gates et diagnostic, sous `DERIVED_PLAYBOOK_V11`, puis les relie par `DERIVED_FROM` aux sources canoniques. Cette migration est hors des hooks et de `aret_prepare_handoff`. La macro écrit transactionnellement le handoff, les adresses, l’état `NONE` ou `ACTIVE` du checkpoint, `last_action` et les deux curseurs d’observations. `NONE` impose les cinq champs techniques vides et le contexte annonce qu’aucun geste ne doit être inventé. `ACTIVE` exige cible, changement, état d’exécution, dernière validation et actions immédiates sous leurs bornes. Les observations V1.3 sont une fenêtre de trois faits maximum, dérivée uniquement des `pipeline_run` et preuves déjà persistés après ces curseurs ; elles ne décrivent jamais une intention, un correctif ou une prochaine action. Une dernière validation qui cite une adresse pipeline/preuve et un verdict contradictoire est refusée ; sans adresse canonique, elle reste explicitement déclarée et non vérifiée. Le `handoff_front_hash` couvre le checkpoint et doit égaler le Front courant, faute de quoi le dossier est `STALE` et le hook échoue bruyamment. Les handoffs sans état V1.2 ou sans curseurs V1.3 échouent volontairement ; aucune migration ne fabrique de checkpoint ni de frontière d’observation a posteriori. Le dossier est borné à 12 500 octets ; le contexte final est borné à 18 500 octets, sans troncature. Après injection, l’agent rédige les six volets du rituel et transmet également `resume_contract_hash`, qui couvre l’instantané d’observations, à `aret_acknowledge_resume`. `PostToolUse` compare cette valeur au hash armé pour la session ; toute divergence maintient la barrière. L’état du garde est éphémère sous `.aret-memory/runtime/resume_guard/`, donc n’est ni une connaissance SQLite ni une donnée Git. Le résumé de compaction, lorsqu’il est fourni, reste un champ d’audit et ne devient pas une connaissance canonique.

## Gouvernance V1.4 : laboratoire, capacités et mémoire durable

Lorsque le catalogue MCP expose une capacité ARET — oracle, différentiel Wine, wall, corpus, asset ou pipeline — l’agent doit l’utiliser plutôt qu’un équivalent shell direct. Le shell reste autorisé pour compilation, exploration, GDB, recherche et prototypes ; une sortie locale ne devient jamais implicitement une preuve, une observation MCP ou une décision canonique. Avant une décision durable, l’agent consulte le Front et les objets nécessaires ; après un fait durable, une décision, une preuve ou un handoff, il écrit seulement l’objet métier approprié, sans stocker le bruit d’exploration.

Un prototype ne doit pas être industrialisé au premier essai. En revanche, tout outil qui devient réutilisable ou contribue de façon récurrente à une décision, une validation, une preuve, un corpus, un asset ou une priorisation doit être intégré au MCP avant d’être présenté comme capacité officielle. Cette intégration requiert un catalogue fermé, des paramètres bornés, une politique, un résultat ou artefact adressable lorsque nécessaire, des tests et une documentation compacte. Elle n’ajoute aucune whitelist de barrière : PreToolUse continue seulement à exiger une reprise correctement acquittée.
