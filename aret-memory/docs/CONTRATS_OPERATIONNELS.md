...
|---|---|
| Hooks | Lecture seule par défaut. `SessionStart` et `PostCompact` injectent un Resume Dossier V1.2 prêt puis arment une barrière liée à son hash ; les checkpoints d’audit exigent `ARET_HOOK_WRITE_ENABLED=true`. |
...
|---|---|---|
| `SessionStart` | `.claude/hooks/aret-mmu-session-start.sh` | Vérifie et injecte le Resume Dossier V1.2, puis arme une barrière liée à son empreinte. |
...
| `PostCompact` | `.claude/hooks/aret-mmu-post-compact.sh` | Réinjecte un Resume Dossier V1.2 fraîchement vérifié et réarme une barrière liée à son empreinte ; trace optionnelle de compaction. |
...
`SessionStart` et `PostCompact` n’injectent ni historique massif ni extraits sélectionnés par date. Ils exigent et formatent un **Resume Dossier V1.2** à six sections fixes : playbook stable, handoff actif avec checkpoint technique, adresses pertinentes, capacités/outils/portes, Git/limites/garde-fous et rituel. Le playbook utilise exclusivement `knowledge` et `knowledge_tag` : ses cinq domaines obligatoires sont `PLAYBOOK_FOUNDATION`, `PLAYBOOK_METHOD`, `PLAYBOOK_ARCHITECTURE` avec `PLAYBOOK_SHARED_STACK`, `PLAYBOOK_GATES` et `PLAYBOOK_TOOLING`, tous sous le tag `CORE_PLAYBOOK`. La migration one-off `migration/bootstrap_playbook_v11.py` ajoute idempotemment trois fiches `RULE` compactes de méthode, gates et diagnostic, sous `DERIVED_PLAYBOOK_V11`, puis les relie par `DERIVED_FROM` aux sources canoniques. Cette migration est hors des hooks et de `aret_prepare_handoff`. La macro écrit transactionnellement le handoff, les adresses, l’état `NONE` ou `ACTIVE` du checkpoint et `last_action`. `NONE` impose les cinq champs techniques vides et le contexte annonce qu’aucun geste ne doit être inventé. `ACTIVE` exige cible, changement, état d’exécution, dernière validation et actions immédiates sous leurs bornes. Le `handoff_front_hash` couvre l’ensemble et doit égaler le Front courant, faute de quoi le dossier est `STALE` et le hook échoue bruyamment. Les handoffs V1.1 sans état V1.2 échouent volontairement ; aucune migration ne fabrique de checkpoint a posteriori. Le dossier est borné à 12 500 octets ; le contexte final est borné à 18 500 octets, sans troncature. Après injection, l’agent rédige les six volets du rituel et transmet également `resume_contract_hash` à `aret_acknowledge_resume`. `PostToolUse` compare cette valeur au hash armé pour la session ; toute divergence maintient la barrière. L’état du garde est éphémère sous `.aret-memory/runtime/resume_guard/`, donc n’est ni une connaissance SQLite ni une donnée Git. Le résumé de compaction, lorsqu’il est fourni, reste un champ d’audit et ne devient pas une connaissance canonique.
