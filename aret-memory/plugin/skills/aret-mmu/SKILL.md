...
3bis. OBLIGATION — Front toujours à jour : le Front est le pointeur de travail vivant. Dès qu’il change (subsystem, brique, mur courant, prochaine action, dernier acte, champ de handoff), le mettre à jour immédiatement via `aret_update_front`. Garder l’avancement fidèle est primordial pour une reprise correcte.
3ter. OBLIGATION — handoff jamais périmé : `aret_update_front` renvoie `handoff_status`. Si `stale=true` (le changement a modifié une clé de reprise), ré-exécuter `aret_prepare_handoff` avant de terminer ou de laisser compacter la session. Vérifier la continuité à tout moment via `aret_get_resume_status` (`degraded=true` ⇒ prochaine reprise dégradée). `aret_prepare_handoff` rend toutes les violations de bornes d’un coup (octets réels vs borne par champ) et, si le dossier assemblé dépasse le budget, un diagnostic COMPACT (`written=true`, `errors`, `overflow_bytes`, `field_bytes`) sans ré-écho du playbook : corriger en un seul aller-retour.
